...
/*
 * create a direct-map page table for any given pagetable，将内核页表的创建与初始化解耦
 */
void
...
walk(pagetable_t pagetable, uint64 va, int alloc)
{
  ...
  {
    pte_t *pte = &pagetable[PX(level, va)]; // 这是个索引之后的物理地址！
    if (*pte & PTE_V)
    { // 有效位
    ...
    {
      if (!alloc || (pagetable = (pde_t *)kalloc()) == 0) // 不需要分配或者分配失败
        return 0;
      memset(pagetable, 0, PGSIZE);     // 分配成功，记录新的页表物理地址并将内容清零
      *pte = PA2PTE(pagetable) | PTE_V; // 将新的页表物理地址写入到当前页表项中
    }
  }
...
void dfs(pagetable_t pagetable, int level)
{
  ...
  {
    pte_t pte = pagetable[i];
    if (!pte & PTE_V) // 无效位直接跳过
      continue;
    printf("..");
    for (int j = level; j < 2; j++)
      printf(" ..");
    printf("%d: pte %p pa %p\n", i, pte, PTE2PA(pte));
    if (level != 0) // 无权限位，说明是页表
    {
      uint64 next = PTE2PA(pte);
      dfs((pagetable_t)next, level - 1);
    }
  }
}
